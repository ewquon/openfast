...
INTEGER, SAVE :: NTimes=-1, itime
REAL(ReKi), ALLOCATABLE, SAVE :: YawTime(:), YawAngle(:)
REAL(ReKi) :: dummyval
CHARACTER(1024) :: dummystr
INTEGER :: istat

if (NTimes < 0) then
    ! read in specified yaw hist
    open(42,file=trim(DirRoot)//'.UserYawCont',status='old',action='read')
    istat = 0
    do while (istat == 0)
        NTimes = NTimes + 1
        read(42,*,iostat=istat) dummyval, dummyval
    end do

    write(*,*) 'Reading user-defined yaw history with ',NTimes,'times'
    allocate(YawTime(NTimes))
    allocate(YawAngle(NTimes))
    rewind(42)
    do itime = 1,NTimes
        read(42,*,iostat=istat) YawTime(itime), YawAngle(itime)
        if (istat /= 0) then
            write(*,*) 'Problem reading itime=',itime
        end if
    end do
    close(42)
    itime = 1
    YawAngle = YawAngle / 57.2957795131
end if

if (ZTime <= YawTime(1)) then
    write(*,*) 'setting NacYaw at t=',ZTime,'from input t=',YawTime(1)
    YawPosCom = YawAngle(1)
elseif (ZTime >= YawTime(NTimes)) then
    write(*,*) 'setting NacYaw at t=',ZTime,'from input t=',YawTime(NTimes)
    YawPosCom = YawAngle(NTimes)
else
    ! Expect YawTime(1) < ZTime < YawTime(NTimes)
    do while (ZTime > YawTime(itime))
        itime = itime + 1
    end do
    ! interpolate between itime and itime-1
    !write(*,*) 'interpolating NacYaw t=',ZTime,'between i=',itime-1,itime,':',YawTime(itime-1:itime),YawAngle(itime-1:itime)
    YawPosCom = YawAngle(itime-1) + (YawAngle(itime)-YawAngle(itime-1)) / (YawTime(itime)-YawTime(itime-1)) * (ZTime-YawTime(itime-1))
end if

YawRateCom = 0.0
!YawRateCom = (YawPosCom - YawPos) / DT
!! report nacyaw update if we're yawing more than 0.5 deg
!if (abs(YawPosCom - YawPos) > 0.00872664626) then
!    write(*,*) ZTime,': NacYaw',YawPos,'-->',YawPosCom
!end if
